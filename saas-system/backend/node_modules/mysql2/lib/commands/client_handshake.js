...
class ClientHandshake extends Command {
  ...
  handshakeResult(packet, connection) {
    ...
    // packet can be OK_Packet, ERR_Packet, AuthRoutesRequest, AuthNextFactor
    // or AuthMoreData
    if (marker === 0xfe || marker === 1 || marker === 0x02) {
      const authRoutes = require('./auth_switch');
      try {
        if (marker === 1) {
          authRoutes.authRoutesRequestMoreData(packet, connection, this);
        } else {
          ...
          // which is similar in structure to an AuthRoutesRequest packet,
          // so, we can use it directly
          authRoutes.authRoutesRequest(packet, connection, this);
        }
        return ClientHandshake.prototype.handshakeResult;
      } catch (err) {
